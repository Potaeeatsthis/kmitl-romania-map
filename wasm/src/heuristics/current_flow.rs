...
// ---------------------------------------------------------------------------
// Explanation / teaching view: same algorithm as current_flow_heuristic above,
// but records the Laplacian, the per-edge conductances, and a matrix snapshot
// after every pivot column so the frontend can walk through Gauss-Jordan
// elimination step by step. Only used by the explanation page -- the actual
// search path still calls current_flow_for_goal(), which reads the
// build-time-precomputed table and never runs this. See ARCHITECTURE_DECISION.md:
// "the browser never runs Gauss-Jordan" refers to that hot path, not this
// explicitly-opened, user-triggered explanation view.
// ---------------------------------------------------------------------------

#[derive(Clone, Debug, PartialEq, Serialize)]
pub struct ConductanceEdge {
    pub city_a: usize,
    pub city_b: usize,
    pub distance: u32,
    pub conductance: f64,
...
pub fn explain_current_flow(
    ...
    for (a, roads) in graph.iter().enumerate() {
        for &(b, distance) in roads {
            ...
            laplacian[a][b] -= conductance;
            if a < b {
                conductances.push(ConductanceEdge {
                    city_a: a,
                    city_b: b,
                    distance,
                    conductance,
    ...
    for column in 0..n {
        ...
        steps.push(EliminationStep {
            pivot_column: column,
            pivot_row: column, // after the swap above, the pivot now lives at `column`
            matrix_after: augmented.clone(),
        });
    }
    ...
    Ok(HeuristicExplanation {
        start,
        goal,
        conductances,
        laplacian,
        steps,
        effective_resistance: values[start],
    })
}
