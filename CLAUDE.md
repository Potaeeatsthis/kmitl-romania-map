...
| `lib/roadPath.ts` | **Working.** 182 lines, 14 tests. Pure geometry helpers for curved road rendering |
...
| `vitest.config.mts`, `vitest.setup.ts` | **Working.** jsdom + testing-library. 44 tests across 6 files |
...
Current coverage is 44 tests in 6 files: trace selectors (6), the wasm JSON boundary (4),
Zustand search/playback state (3), route-selection interactions (12), the benchmark panel (5),
road geometry helpers (14).
...
npm test                    # vitest run — 44 frontend tests
