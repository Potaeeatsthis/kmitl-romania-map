...
| `lib/roadPath.ts` | **Working.** 182 lines, 16 tests. Pure geometry helpers for curved road rendering |
...
| `components/benchmark/` | **Working.** `BenchmarkPanel.tsx` (75 lines, 6 tests) and `BenchmarkCharts.tsx` (381 lines) |
...
| `vitest.config.mts`, `vitest.setup.ts` | **Working.** jsdom + testing-library. 49 tests across 6 files |
...
Current coverage is 49 tests in 6 files: trace selectors (6), the wasm JSON boundary (4),
Zustand search/playback state (5), route-selection interactions (12), the benchmark panel (6),
road geometry helpers (16).
...
npm test                    # vitest run — 49 frontend tests
